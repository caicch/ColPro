...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor, start_pos: int, freqs_cis: torch.Tensor, mask: Optional[torch.Tensor], adapter=None, video_start=None):
        ...
        if adapter is not None:
            return self.wo(output), xk[:, -adapter_len:,]
        else:
            return self.wo(output), xk
...
class Transformer(nn.Module):
    def __init__(self, params: ModelArgs, args):
        ...
        self.adapter_query_L2P = Embedding(params.adapter_len * params.adapter_layer, params.dim)
    ...
    def forward(self, data, inference=False):
        ...
        adapter_L2P = self.adapter_query_L2P.weight.reshape(-1, self.adapter_len, self.params.dim).unsqueeze(1)
